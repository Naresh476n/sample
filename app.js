...
for (let i = 1; i <= 4; i++) {
  ...
  if (el) {
    el.addEventListener("change", (e) => toggleRelay(i, e.target.checked));
  }
}
...
document.getElementById("saveLimits").addEventListener("click", () => {
  for (let i = 1; i <= 4; i++) {
    const v = parseFloat(document.getElementById(`limit${i}`).value);
...
function updateLiveDemo() {
  ...
  let validVoltages = voltages.filter((v) => parseFloat(v) > 0);
  let avgVoltage =
...
// -----------------------------
// IMPORTANT: keep these values secure. Replace with your own if needed.
...
document.getElementById("loadCharts").addEventListener("click", async () => {
  ...
  try {
    if (selected === "day") {
      // Ensure a valid date string
      const dayInput = document.getElementById("singleDay");
      const day = (dayInput && dayInput.value) ? dayInput.value : new Date().toISOString().split("T")[0];
      if (dayInput && !dayInput.value) dayInput.value = day; // set UI to today if empty
      ...
      if (loads) {
        ...
        // If no row exists, insert the sample row the user provided (1.5,1.5,1.4,1.6)
    ...
    alert("Failed to load data. Please check Supabase URL/key, table schema, and browser console for details.");
